...
export async function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('Blip.summarize', async () => {
		...
		var url = vscode.workspace.getConfiguration().get('Blip.url');
		if (!url) {
			vscode.window.showInputBox({}).then(
				function (result) {
					vscode.workspace.getConfiguration().update('Blip.url', result);
					url = result;
				}
			);
		}
